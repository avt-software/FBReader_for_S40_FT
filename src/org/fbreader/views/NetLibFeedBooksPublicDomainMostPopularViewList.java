...
public final class NetLibFeedBooksPublicDomainMostPopularViewList 
                    extends FancyCustomList 
                    implements View, CommandListener {

    private static NetLibFeedBooksPublicDomainMostPopularViewList self = null;
    ...
    private final FeedBooksEbookModel feedBooksPublicDomainMostPopularModel = new FeedBooksEbookModel(100);
    private String viewTitleStr;

    public NetLibFeedBooksPublicDomainMostPopularViewList(MIDlet parent) {
         super("");
        ...
        viewMaster.setFirsLoadFeedBooksPublicDomainMostPopularList(true);
    }
     ...
     */
    public static NetLibFeedBooksPublicDomainMostPopularViewList getInstance() {
        if (self == null) {
            self = new NetLibFeedBooksPublicDomainMostPopularViewList(Main.getInstance());
        }
        return self;
    }    
     ...
     /**
     * Activate NetLibFeedBooksPublicDomainMostPopularViewList 
     */
    public void activate() {
        ...
        if (viewMaster.getFirsLoadFeedBooksPublicDomainMostPopularList()) {
            reload();
        }
        else if (feedBooksPublicDomainMostPopularModel.size()!=0) {
            /** 
    ...
    /**
     * Deactivate NetLibFeedBooksPublicDomainMostPopularViewList
     * 
     */
...
//--- 2 --- 
            FeedBooksEbook selectedFeedBooksPublicDomainMostPopularListItem=                   
                           feedBooksPublicDomainMostPopularModel.getFeedBooksEbookByIdAt(this.getSelectedIndex());
            selectedBookForDownload.setBookTitle(selectedFeedBooksPublicDomainMostPopularListItem.getTitle());
            selectedBookForDownload.setBookAuthor(selectedFeedBooksPublicDomainMostPopularListItem.getName());
            selectedBookForDownload.setBookTags(selectedFeedBooksPublicDomainMostPopularListItem.getCategory());
            selectedBookForDownload.setBookLang(selectedFeedBooksPublicDomainMostPopularListItem.getLanguage());
            String bookURL=selectedFeedBooksPublicDomainMostPopularListItem.getLink();
             ...
             new Task() {
                public Object doInBackground(final Object in) { 
                    ...
                    if (!httpGetter.isCanceled()) {
                        try {
                                   feedBooksPublicDomainMostPopularModel.removeAllElements();

                                   synchronized (Worker.LARGE_MEMORY_MUTEX) {
                                       feedBooksPublicDomainMostPopularModel.setXML((byte[]) in);
                                   }
                        } catch (Exception e) {
                                       //#debug
                                       Main.LOGGER.logError("Error parsing XML" + feedBooksPublicDomainMostPopularModel.toString());
                        }

                        //hide loading process window 
                        Main.getInstance().switchDisplayable(null,viewMaster.getNetLibFeedBooksPublicDomainMostPopularViewList());
                        final int feedBooksPublicDomainCatalogModelSize = feedBooksPublicDomainMostPopularModel.size();
                        ...
                        if (feedBooksPublicDomainCatalogModelSize!=0 ) {
                            ...
                            viewMaster.setFirsLoadFeedBooksPublicDomainMostPopularList(false);                          
    ...
    private void paint() {
        ...
        final int  feedBooksPublicDomainMostPopularModelSize = feedBooksPublicDomainMostPopularModel.size();
        for (int i = 0; i < feedBooksPublicDomainMostPopularModelSize; i++) {
            addFeedBooksPublicDomainMostPopularListItem(feedBooksPublicDomainMostPopularModel.getFeedBooksEbookByIdAt(i));
        }
    ...
    public void addFeedBooksPublicDomainMostPopularListItem(final FeedBooksEbook feedBooksPublicDomainMostPopularListItem) {
        final String bookTitle=feedBooksPublicDomainMostPopularListItem.getTitle().toLowerCase();
        final String bookAuthor=feedBooksPublicDomainMostPopularListItem.getName().toLowerCase();
